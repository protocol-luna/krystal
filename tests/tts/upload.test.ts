...
describe("requestUploadUrl", () => {
	...
	it("throws on non-ok response", async () => {
		...
		await expect(requestUploadUrl("c1", 1000, 5)).rejects.toThrow("attachments POST 403");
		globalThis.fetch = originalFetch;
	});

	it("throws on missing upload_url in response", async () => {
		const originalFetch = globalThis.fetch;
		globalThis.fetch = async () => new Response(JSON.stringify({ attachments: [{}] }), { status: 200 });
		const { requestUploadUrl } = await import("../../src/tts/upload.js");
		await expect(requestUploadUrl("c1", 1000, 5)).rejects.toThrow("URL d'upload");
		globalThis.fetch = originalFetch;
	});
	...
	it("returns upload URL and filename on success", async () => {
		...
		globalThis.fetch = async () => new Response(
			JSON.stringify({ attachments: [{ upload_url: "https://cdn.example.com/upload", upload_filename: "voice.ogg" }] }),
			{ status: 200 },
		);
		const { requestUploadUrl } = await import("../../src/tts/upload.js");
...
describe("putFileToUploadUrl", () => {
	...
	it("throws on non-ok response", async () => {
		...
		await expect(putFileToUploadUrl("https://cdn.example.com/upload", Buffer.from("test"))).rejects.toThrow("PUT upload 400");
		globalThis.fetch = originalFetch;
	});
	...
	it("succeeds on ok response", async () => {
		...
		const { putFileToUploadUrl } = await import("../../src/tts/upload.js");
		await putFileToUploadUrl("https://cdn.example.com/upload", Buffer.from("test"));
		globalThis.fetch = originalFetch;
	});
});
...
describe("postVoiceMessage", () => {
	...
	it("throws on non-ok response", async () => {
		...
		await expect(postVoiceMessage("c1", "voice.ogg", 5, "dGVzdA==")).rejects.toThrow("messages POST 429");
		globalThis.fetch = originalFetch;
	});
