...
describe("applyTypoCorrection", () => {
	...
	it("edits message for edit style", async () => {
		...
		const { applyTypoCorrection } = await import("../../src/bot/typo-correction.js");
		let edited = false;
		const client = {
			editMessage: async () => { edited = true; },
			createMessage: async () => {},
		};
		await applyTypoCorrection(client as any, "c1", "m1", { chunkIndex: 0, original: "hello world", correctedWord: "hello" });
	...
	it("creates correction message for message style", async () => {
		...
		const client = {
			editMessage: async () => {},
			createMessage: async (_id: string, opts: any) => { created = opts.content; },
		};
		await applyTypoCorrection(client as any, "c1", "m1", { chunkIndex: 0, original: "hello world", correctedWord: "hello" });
...
describe("resolveStyle", () => {
	it("returns edit when config is edit", async () => {
		...
		const client = {
			editMessage: async () => { style = "edit"; },
			createMessage: async () => { style = "message"; },
		};
		await applyTypoCorrection(client as any, "c1", "m1", { chunkIndex: 0, original: "test", correctedWord: "test" });
	...
	it("handles errors gracefully", async () => {
		...
		const { applyTypoCorrection } = await import("../../src/bot/typo-correction.js");
		const client = {
			editMessage: async () => { throw new Error("fail"); },
			createMessage: async () => {},
		};
		// Should not throw
		await applyTypoCorrection(client as any, "c1", "m1", { chunkIndex: 0, original: "test", correctedWord: "test" });
