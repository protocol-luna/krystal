...
describe("llmBus", () => {
	...
	it("can emit and receive token events", async () => {
		const { llmBus } = await import("../../src/core/llm-bus.js");
		let received = "";
		llmBus.once("token", (chunk: string) => { received = chunk; });
	...
	it("can emit and receive done events", async () => {
		const { llmBus } = await import("../../src/core/llm-bus.js");
		let text = "";
		llmBus.once("done", (full: string) => { text = full; });
	...
	it("can emit and receive error events", async () => {
		...
		llmBus.once("error", (e: Error) => { err = e; });
	...
	it("can emit and receive crash events", async () => {
		...
		llmBus.once("crash", (c: number | null) => { code = c; });
		llmBus.emit("crash", 1);
		expect(code).toBe(1);
	});
